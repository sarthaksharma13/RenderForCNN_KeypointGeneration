...
% MODIFIED FOR REALISTIC ANGELS FOR CAR CLASS.

% erange = linspace(min(elevations), max(elevations)+1e-5, 51);
erange = linspace(0,3,51);
he(end) = [];
...
% hack for elevation
goodSamples(:, 2) = int32(mod(goodSamples(:, 2)+90, 180)/3) + 15;
...
% randElevation = max(min(normrnd(outlier_elevation_avg, outlier_elevation_std, numRandPerturbation, 1), 85),1e-5);
randElevation = max(min(normrnd(outlier_elevation_avg, outlier_elevation_std, numRandPerturbation, 1), 75),60);

% MODIFIED FOR REALISTIC ANGELS FOR CAR CLASS.
% randTilt = max(min(normrnd(outlier_tilt_avg, outlier_tilt_std, numRandPerturbation, 1), 45),-45);
randTilt = max(min(normrnd(outlier_tilt_avg, outlier_tilt_std, numRandPerturbation, 1), 45),-10);
